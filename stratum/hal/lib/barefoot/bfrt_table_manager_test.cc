...
TEST_F(BfrtTableManagerTest,  ReadActionProfileMemberTest) {
 ...
 { 
    ...
    EXPECT_CALL(*bf_sde_wrapper_mock_, GetActionProfileMembers(kDevice1, _, kBfRtTableId, member_id, _, _ ))
        .WillOnce(DoAll(SetArgPointee<4>(member_ids), Return(::util::OkStatus())));

    EXPECT_CALL(*bf_sde_wrapper_mock_, GetP4InfoId(kBfRtTableId))
        .WillOnce(Return(kP4TableId));
  ...
  EXPECT_OK(
    bfrt_table_manager_->ReadActionProfileMember(session_mock, entry, &writer_mock));
}
